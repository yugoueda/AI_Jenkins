        ...
        with db.engine.begin() as conn:
            ...
            conn.execute(
                text(
                    ...
                    "repository_url, status, created_at) "
                    "VALUES (:project_id, :mr_id, :commit_sha, :source_branch, "
                    ":target_branch, :repository_url, :status, CURRENT_TIMESTAMP)"
                ),
                {
...
def mark_ci_run_running(project_id: str, mr_id: str, commit_sha: str) -> None:
    db.execute(
...
def mark_ci_run_completed(project_id: str, mr_id: str, commit_sha: str) -> None:
    if not commit_sha:
        return
    db.execute(
...
def mark_ci_run_failed(project_id: str, mr_id: str, commit_sha: str) -> None:
    db.execute(
    ...
    with db.engine.begin() as conn:
        ...
        row = (
            conn.execute(
                text(
                    ...
                    "target_branch, repository_url FROM ci_runs "
                    "WHERE project_id=:project_id AND mr_id=:mr_id "
        ...
        updated = conn.execute(
            text(
                ...
                "WHERE project_id=:project_id AND mr_id=:mr_id "
                "AND commit_sha=:commit_sha AND status='PENDING'"
            ),
            dict(row),
        )
